...
const App = () => {
  ...
  const addItem = (
    ...
    let price: number =
      pricePerKilo * weight + pricePerVolume * width * height * depth;
    let weightByVolume = weight / (width * height * depth);
    if (type === 1) {
      weightByVolume *= 2;
    }
    let hint = "";
    if (weightByVolume < 0.00003 && type == 0) {
      hint =
        "Det verkar som denna låda är lätt, överväg att fylla lådan helt för att spara pengar.";
    }
    ...
    <Provider>
      <Portal>
        <SafeAreaView style={{ flex: 1 }}>
          ...
          >
            <EstimationInput submit={addItem} />
          </Modal>
